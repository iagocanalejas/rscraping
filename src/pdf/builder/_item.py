...
from src.pdf.builder._validators import is_valid_dni, is_valid_email
...
def prompt_rower_data(data: PDFItem) -> PDFItem:
    answers = inquirer.prompt([
        ...
        inquirer.Text("birth", message="Fecha nacimiento (DD/MM/YYYY)", validate=lambda _, x: len(x.split('/')) == 3),
...
def prompt_extra_data(data: PDFItem) -> PDFItem:
    answers = inquirer.prompt([
        ...
        inquirer.Text("sign_in", message="Firmado en", default='A POBRA DO CARAMIÑAL', validate=lambda _, x: bool(x)),
    ])
...
def prompt_entity_data(data: PDFItem) -> PDFItem:
    answers = inquirer.prompt([
        ...
        inquirer.Text("email", message="Correo", default='remopuebla@gmail.com', validate=lambda _, x: is_valid_email(x)),
        inquirer.Text("entity", message="Club", default='CLUB REMO PUEBLA', validate=lambda _, x: bool(x)),
        inquirer.Text("entity_town", message="Localidad Club", default='A POBRA DO CARAMIÑAL', validate=lambda _, x: bool(x)),
