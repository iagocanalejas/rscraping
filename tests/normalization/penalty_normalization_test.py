...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"VILA DE CANGAS": Penalty(disqualification=False, reason=STARBOARD_TACK, penalty=0)},
        ),
        (
        ...
        (
            ...
            {"RÍA DE MARÍN": Penalty(disqualification=False, reason=STARBOARD_TACK, penalty=0)},
        ),
        (
            "Fortuna realizió una ciaboga por estribor",
            {"FORTUNA": Penalty(disqualification=False, reason=STARBOARD_TACK, penalty=0)},
        ),
        (
            "Castro había dado la tercera ciaboga por estribor, siendo su tiempo de 21:48.3",
            {"CASTRO": Penalty(disqualification=False, reason=STARBOARD_TACK, penalty=0)},
        ),
        (
            "Castro ocupó la séptima posición pero fue descalificado por realizar la segunda ciaboga por estribor. Su tiempo había sido de 21:48.12.",
            {"CASTRO": Penalty(disqualification=True, reason=STARBOARD_TACK, penalty=0)},
        ),
        (
        ...
        (
            ...
            {"HONDARRIBIA": Penalty(disqualification=False, reason=STARBOARD_TACK, penalty=0)},
        ),
        (
            "El tiempo de Algorta había sido de 20:08.0. Castro había dado la tercera ciaboga por estribor, siendo su tiempo de 21:48.3.",
            {"CASTRO": Penalty(disqualification=False, reason=STARBOARD_TACK, penalty=0)},
        ),
        (
            "Kaiku fue descalificado por realizar una ciaboga de forma incorrecta. Castropol estrenó una trainera de fibra, la primera de los astilleros Amilibia. Se recogió por la mañana en Orio y se remó por la tarde.",
            {"KAIKU": Penalty(disqualification=True, reason=STARBOARD_TACK, penalty=0)},
        ),
    ],
)
def test_starboard_tack_normalization(notes, expected) -> None:
    participants = ["VILA DE CANGAS", "RÍA DE MARÍN", "FORTUNA", "CASTRO", "HONDARRIBIA", "ALGORTA", "KAIKU"]
    assert normalize_penalty(notes, participants) == expected
...
@pytest.mark.parametrize(
    ...
    [
        (
            "San Simón y A Cabana pasaron la baliza de meta por estribor. El tiempo de San Simón había sido de 24:34.29 y el de A Cabana de 24:30.41.",
            {
                "SAN SIMÓN": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0),
                "CABANA": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0),
            },
        ),
        (
            "Cesantes y el Náutico de Vigo dejaron la baliza de meta por estribor. El tiempo de Cesantes había sido de 21:52.58 y el del Náutico de Vigo de 21:39.47.",
            {
                "CESANTES": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0),
                "NÁUTICO DE VIGO": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0),
            },
        ),
        (
            "Castropol entró a meta fuera de línea (la línea de meta entre la baliza uno y la cinco). Su tiempo había sido de 24:52.57.",
            {"CASTROPOL": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0)},
        ),
        (
            "Tirán fue descalificada por entrar en meta dejando la boya por estribor. Su tiempo había sido de 21:58.00.",
            {"TIRÁN": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0)},
        ),
        (
        ...
        (
            "El tiempo de Tirán había sido 19:15.32, pero fue descalificado por dejar su baliza por estribor a la entrada a meta.",
            {"TIRÁN": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0)},
        ),
        (
            "Hondarribia fue descalificado por entrar en la baliza de Pedreña. Había realizado un tiempo de 21:57.",
            {"HONDARRIBIA": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0)},
        ),
        (
            "Rianxo fue descalificado por dejar el puente por el arco equivocado. Su tiempo había sido de 18:58.91. El ganador de la bandera era quien tuviese menos tiempo en la jornada final",
            {"RIANXO": Penalty(disqualification=True, reason=OFF_THE_FIELD, penalty=0)},
        ),
    ],
)
def test_off_the_field_normalization(notes, expected) -> None:
    participants = ["SAN SIMÓN", "CABANA", "CESANTES", "NÁUTICO DE VIGO", "CASTROPOL", "TIRÁN", "HONDARRIBIA", "RIANXO"]
    assert normalize_penalty(notes, participants) == expected
...
def test_wrong_route_normalization(notes, expected) -> None:
    participants = ["ORIO"]
    assert normalize_penalty(notes, participants) == expected
...
def test_lack_of_competitiveness_normalization(notes, expected) -> None:
    participants = ["ONDARROA"]
    assert normalize_penalty(notes, participants) == expected
...
def test_null_start_normalization(notes, expected) -> None:
    participants = ["VIRXE DA GUÍA", "TIRÁN", "FORTUNA", "SAN JUAN"]
    assert normalize_penalty(notes, participants) == expected
...
def test_collision_normalization(notes, expected) -> None:
    participants = [
        "RIANXO",
        "CABO DA CRUZ",
        "SAMERTOLAMEU",
...
def test_wrong_lineup_normalization(notes, expected) -> None:
    participants = ["CHAPELA", "BUEU", "RASPAS", "DEUSTO - PORTUGALETE B", "KAIKU"]
    assert normalize_penalty(notes, participants) == expected
...
def test_doping_normalization(notes, expected) -> None:
    participants = ["YOLA"]
    assert normalize_penalty(notes, participants) == expected
...
def test_sinking_normalization(notes, expected) -> None:
    participants = ["ARES", "PERILLO", "RASPAS"]
    assert normalize_penalty(notes, participants) == expected
...
def test_boat_weight_normalization(notes, expected) -> None:
    participants = ["SAMERTOLAMEU"]
    assert normalize_penalty(notes, participants) == expected
...
def test_unknown_penalty_normalization(notes, expected) -> None:
    participants = ["DONOSTIA ARRAUN LAGUNAK", "SANTOÑÉS"]
    assert normalize_penalty(notes, participants) == expected
...
@pytest.mark.parametrize(
    ("notes", "expected"),
    [
        ("El tiempo de Vilaxoan fue de 26:16.00.", {"VILAXOAN": time(0, 26, 16)}),
        ("El tiempo de Raspas había sido 23:41.86.", {"RASPAS": time(0, 23, 41, 860000)}),
        ("El tiempo de Arkote había sido de 21:48.40.", {"ARKOTE": time(0, 21, 48, 400000)}),
        ("El tiempo de Donostia Arraun Lagunak había sido de 22:19.00.", {"DONOSTIA ARRAUN LAGUNAK": time(0, 22, 19)}),
        (
        ...
        (
            "El tiempo de Amegrove había sido de 20:09.33, pero fue descalificada por invasión de la calle del Náutico de Vigo a la salida de la segunda ciaboga.",
            {"AMEGROVE": time(0, 20, 9, 330000)},
        ),  # noqa: E501
        (
            "El tiempo de Camargo había sido de 21:02.84 y el de Pedreña de 21:09.46. Fueron descalificados por no dar el peso mínimo de la trainera.",
            {"CAMARGO": time(0, 21, 2, 840000), "PEDREÑA": time(0, 21, 9, 460000)},
        ),  # noqa: E501
        (
            "El tiempo final de Camargo había sido de 21:48,00, pero tenía en su tripulación un juvenil que ya había remado tres regatas en el mismo año.",
        ...
        ("Arraun Lagunak tuvo un tiempo de 21:21.02.", {"ARRAUN LAGUNAK": time(0, 21, 21, 20000)}),
        ("Hondarribia había realizado un tiempo de 20:29.1.", {"HONDARRIBIA": time(0, 20, 29, 100000)}),
        (
        ...
        (
            "Donibaneko B y San Juan B participaban en una tanda de promoción, por eso aparecen fuera de regata. El tiempo de Donibaneko B ue de 20:40.36, mientras que San Juan B hizo 20:42.84. Antes",
        ...
        ("Donostia Arraun Lagunak fue descalificado, su tiempo había sido de 20:58.16.", {"": time(0, 20, 58, 160000)}),
        (
            "La primera jornada se tuvo que anular por el mal estado de la mar. El ganador se adjudicó también el III trofeo Manolo Santamaría y la VIII Copa del Rey. La primera tanda sirvió como Campeonato Regional. Santoñés realizó un tiempo de 20:33.0, pero fue descalificado.",
        ...
        (
            'San Juan debería haber salido al minuto de la "Lugañene", pero no se presentó en la baliza de salida, al parecer por un malentendido de su delegado. El tiempo fue de 21:19.85.',
            {"": time(0, 21, 19, 850000)},
        ),  # noqa: E501
        (
            "Samertolameu estorbó a Bueu en la tercera virada. Su tiempo había sido el mejor con 19:07.89.",
        ...
        (
            "En la segunda jornada Donibaneko fue descalificado por invadir la calle de Santurtzi en la segunda ciaboga. Terminó con un tiempo de 20:59,35.",
            {"": time(0, 20, 59, 350000)},
        ),  # noqa: E501
        (
            "Chapela quedó fuera de regata después de una reclamación por problemas con la ficha de uno de sus remeros (Guta Ionut). Perdió la cuarta plaza tras realizar un tiempo de 20:40.22.",
            {"": time(0, 20, 40, 220000)},
        ),  # noqa: E501
        (
            "Rianxo fue descalificado por dejar el puente por el arco equivocado. Su tiempo había sido de 18:58.91. El ganador de la bandera era quien tuviese menos tiempo en la jornada final",
...
def test_retrieve_participant_times(notes, expected) -> None:
    assert retrieve_penalty_times(notes) == expected
...
def test_cancelled(notes) -> None:
    assert is_cancelled(notes)
...
def test_retired(participant, notes) -> None:
    assert is_retired(participant, notes)
...
def test_guest(participant, notes) -> None:
    assert is_guest(participant, notes)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "LAREDO",
            "Estaba prevista la participación de Laredo pero no se presentó.",
        ),
    ],
)
def test_absent(participant, notes) -> None:
    assert is_absent(participant, notes)
...
def test_no_penalty(notes) -> None:
    assert normalize_penalty(notes, []) == {}
