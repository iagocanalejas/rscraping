...
class ARCClient(Client, source=Datasource.ARC):
    ...
    def get_race_by_id(self, race_id: str, is_female: bool, **_) -> Optional[Race]:
        ...
        race = Race(
            ...
            race_lanes=self.get_race_lanes(),
            participants=[],
        )
    ...
    def get_name(self) -> str:
        return whitespaces_clean(self._selector.xpath('//*[@id="main"]/div[2]/div[1]/h2/text()').get("")).upper()

    def get_date(self) -> date:
        value = whitespaces_clean(
            self._selector.xpath('//*[@id="main"]/div[2]/div[2]/div[1]/div[1]/ul/li[1]/text()').get("")
        )
    ...
    def get_league(self, is_female: bool) -> Optional[str]:
        ...
        text = whitespaces_clean(self._selector.xpath('//*[@id="main"]/h1/span/text()').get("")).upper()
        if is_play_off(text):
    ...
    def get_town(self) -> str:
        text = remove_parenthesis(
            self._selector.xpath('//*[@id="main"]/div[2]/div[2]/div[1]/div[1]/ul/li[4]/text()').get("")
        )
        text = text.replace(" Ver mapaOcultar mapa", "")
        return whitespaces_clean(text).upper()

    def get_race_lanes(self) -> int:
        text = self._selector.xpath('//*[@id="main"]/div[2]/div[2]/div[1]/div[1]/ul/li[3]/text()').get("")
    ...
    def is_cancelled(self) -> bool:
        ...
        return (
            "NO PUNTUABLE"
            in whitespaces_clean(self._selector.xpath('//*[@id="main"]/div[2]/div[1]/h2/text()').get("")).upper()
        )

    def get_participants(self) -> List[Selector]:
        return [
            Selector(p)
            for p in self._selector.xpath('//*[@id="widget-resultados"]/div/div[3]/div/table[*]/tbody/tr').getall()
        ]
    ...
    def get_series(self, participant: Selector) -> int:
        series = 1
        rows = [Selector(p) for p in self._selector.xpath('//*[@id="widget-resultados"]/div/div[3]/div/table').getall()]
        for row in rows:
